...
const CLIENT_ACTION_TOKEN_TTL_SECONDS=60 * 60 * 24 * 30;
...
function signClientActionToken({ sid, email, company = '', action, exp = null, now = Date.now() } = {}, env = process.env) {
  const secret = tokenSecret(env);
  if (!secret) throw new Error('Missing client action token signing secret.');
  const payload = { sid: String(sid || '').trim(), email: cleanEmail(email), company: String(company || '').trim(), action: cleanAction(action), exp: Number(exp || Math.floor(now / 1000) + CLIENT_ACTION_TOKEN_TTL_SECONDS) };
  if (!payload.sid || !payload.email) throw new Error('Missing client action token subject.');
  const payloadB64 = b64url(JSON.stringify(payload));
  const sig = crypto.createHmac('sha256', secret).update(payloadB64).digest('base64url');
  return `${payloadB64}.${sig}`;
}
function verifyClientActionToken(token, { action = null, now = Date.now() } = {}, env = process.env) {
  const secret = tokenSecret(env);
...
module.exports = { CLIENT_ACTION_TOKEN_TTL_SECONDS, tokenSecret, signClientActionToken, verifyClientActionToken };
