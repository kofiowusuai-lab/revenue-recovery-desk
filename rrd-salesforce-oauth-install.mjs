#!/usr/bin/env node
/** Local-only Salesforce OAuth credential installer.
...
const PORT = Number(process.env.RRD_SALESFORCE_OAUTH_INSTALL_PORT || 8793);
...
function html(message = '') {
  return `<!doctype html><html><head><meta charset="utf-8"><title>RRD Salesforce OAuth Installer</title>
  <style>
  ...
  </head><body><main><h1>Install Salesforce OAuth credentials</h1>${message}
  ...
  <p class="hint">Paste the <b>Consumer Key / Client ID</b> and <b>Consumer Secret / Client Secret</b> from the Salesforce External Client App for <b>Revenue Recovery Desk</b>.</p>
  <form method="post" action="/save?token=${TOKEN}">
    <label>Salesforce Consumer Key / Client ID</label>
    <input name="client_id" autocomplete="off" spellcheck="false" autofocus>
    <label>Salesforce Consumer Secret / Client Secret</label>
    <input name="client_secret" type="password" autocomplete="off" spellcheck="false" placeholder="paste secret here">
    <button type="submit">Install Salesforce credentials</button>
  </form></main></body></html>`;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/save') {
      ...
      if (!client_id || !client_secret) throw new Error('Missing Salesforce client ID or client secret');
      const backup = upsertEnv({
        SALESFORCE_OAUTH_CLIENT_ID: client_id,
        SALESFORCE_OAUTH_CLIENT_SECRET: client_secret,
        SALESFORCE_CLIENT_ID: client_id,
        SALESFORCE_CLIENT_SECRET: client_secret,
      });
      console.log(JSON.stringify({ ok: true, installed: ['SALESFORCE_OAUTH_CLIENT_ID','SALESFORCE_OAUTH_CLIENT_SECRET','SALESFORCE_CLIENT_ID','SALESFORCE_CLIENT_SECRET'], env: ENV_PATH, backupCreated: fs.existsSync(backup) }));
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      res.end(html(`<p class="ok"><b>Installed.</b> Salesforce OAuth credentials were written locally. You can close this tab and reply “done”.</p>`));
...
server.listen(PORT, HOST, () => {
  ...
  fs.writeFileSync('/tmp/rrd-salesforce-oauth-install-url.txt', url + '\n', { mode: 0o600 });
