#!/usr/bin/env node
/** Local-only Intuit/QuickBooks OAuth credential installer.
...
const PORT = Number(process.env.RRD_INTUIT_OAUTH_INSTALL_PORT || 8791);
...
function html(message = '') {
  return `<!doctype html><html><head><meta charset="utf-8"><title>RRD Intuit OAuth Installer</title>
  <style>
  ...
  </head><body><main><h1>Install Intuit / QuickBooks OAuth credentials</h1>${message}
  <p class="warn"><b>Local-only:</b> this page is served from this Mac at <code>127.0.0.1</code> and writes directly to <code>/Users/AIAgenterminal/.openclaw/.env</code>. Do not paste these credentials into Telegram.</p>
  <p class="hint">Paste the <b>Client ID</b> and <b>Client Secret</b> from the Intuit Developer app for <b>Revenue Recovery Desk</b>.</p>
  <form method="post" action="/save?token=${TOKEN}">
    <label>Intuit / QuickBooks Client ID</label>
    <input name="client_id" autocomplete="off" spellcheck="false" autofocus>
    <label>Intuit / QuickBooks Client Secret</label>
    <input name="client_secret" type="password" autocomplete="off" spellcheck="false" placeholder="paste secret here">
    <button type="submit">Install Intuit credentials</button>
  </form></main></body></html>`;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/save') {
      ...
      const client_id = String(params.get('client_id') || '').trim();
      const client_secret = String(params.get('client_secret') || '').trim();
      if (!client_id || !client_secret) throw new Error('Missing Intuit client ID or client secret');
      const backup = upsertEnv({
        INTUIT_OAUTH_CLIENT_ID: client_id,
        INTUIT_OAUTH_CLIENT_SECRET: client_secret,
        QUICKBOOKS_OAUTH_CLIENT_ID: client_id,
        QUICKBOOKS_OAUTH_CLIENT_SECRET: client_secret,
      ...
      console.log(JSON.stringify({ ok: true, installed: ['INTUIT_OAUTH_CLIENT_ID','INTUIT_OAUTH_CLIENT_SECRET','QUICKBOOKS_OAUTH_CLIENT_ID','QUICKBOOKS_OAUTH_CLIENT_SECRET','INTUIT_CLIENT_ID','INTUIT_CLIENT_SECRET','QUICKBOOKS_CLIENT_ID','QUICKBOOKS_CLIENT_SECRET'], env: ENV_PATH, backupCreated: fs.existsSync(backup) }));
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      res.end(html(`<p class="ok"><b>Installed.</b> Intuit / QuickBooks OAuth credentials were written locally. You can close this tab and reply “done”.</p>`));
...
server.listen(PORT, HOST, () => {
  ...
  fs.writeFileSync('/tmp/rrd-intuit-oauth-install-url.txt', url + '\n', { mode: 0o600 });
