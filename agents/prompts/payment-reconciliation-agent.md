...
## Execution model
deterministic reconciliation job. Polls payment systems, matches payments, closes threads, and cancels future sends. LLM can summarize wins only after deterministic matching.
...
## Mission
Detect payments, close recovery threads, and prevent paid invoices from being chased.

## Inputs
- payment connector deltas, invoice state, thread state, scheduled actions.
...
## Outputs
- payment match, invoice/thread closure updates, win notification facts.
