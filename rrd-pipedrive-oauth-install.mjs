#!/usr/bin/env node
/** Local-only Pipedrive OAuth credential installer.
...
const PORT = Number(process.env.RRD_PIPEDRIVE_OAUTH_INSTALL_PORT || 8792);
...
function html(message = '') {
  return `<!doctype html><html><head><meta charset="utf-8"><title>RRD Pipedrive OAuth Installer</title>
  <style>
  ...
  </head><body><main><h1>Install Pipedrive OAuth credentials</h1>${message}
  ...
  <p class="hint">Paste the <b>Client ID</b> and <b>Client Secret</b> from the Pipedrive developer app for <b>Revenue Recovery Desk</b>.</p>
  <form method="post" action="/save?token=${TOKEN}">
    <label>Pipedrive Client ID</label>
    <input name="client_id" autocomplete="off" spellcheck="false" autofocus>
    <label>Pipedrive Client Secret</label>
    <input name="client_secret" type="password" autocomplete="off" spellcheck="false" placeholder="paste secret here">
    <button type="submit">Install Pipedrive credentials</button>
  </form></main></body></html>`;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/save') {
      ...
      if (!client_id || !client_secret) throw new Error('Missing Pipedrive client ID or client secret');
      const installed = [
      ...
      const backup = upsertEnv({
        PIPEDRIVE_OAUTH_CLIENT_ID: client_id,
        PIPEDRIVE_OAUTH_CLIENT_SECRET: client_secret,
        PIPEDRIVE_CLIENT_ID: client_id,
        PIPEDRIVE_CLIENT_SECRET: client_secret,
      });
      console.log(JSON.stringify({ ok: true, installed, env: ENV_PATH, backupCreated: fs.existsSync(backup) }));
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      res.end(html(`<p class="ok"><b>Installed.</b> Pipedrive OAuth credentials were written locally. You can close this tab and reply “done”.</p>`));
...
server.listen(PORT, HOST, () => {
  ...
  fs.writeFileSync('/tmp/rrd-pipedrive-oauth-install-url.txt', url + '\n', { mode: 0o600 });
