#!/usr/bin/env node
/** Local-only Zoho OAuth credential installer.
...
const PORT = Number(process.env.RRD_ZOHO_OAUTH_INSTALL_PORT || 8790);
...
function html(message = '') {
  return `<!doctype html><html><head><meta charset="utf-8"><title>RRD Zoho OAuth Installer</title>
  <style>
  ...
  </head><body><main><h1>Install Zoho OAuth credentials</h1>${message}
  ...
  <p class="hint">Paste the Client ID and Client Secret from the Zoho API Console <b>Server-based Application</b> for <b>Revenue Recovery Desk</b>.</p>
  <form method="post" action="/save?token=${TOKEN}">
    <label>Zoho Client ID</label>
    <input name="client_id" autocomplete="off" spellcheck="false" placeholder="1000.xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" autofocus>
    ...
    <button type="submit">Install Zoho credentials</button>
  </form></main></body></html>`;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/save') {
      ...
      if (!client_id || !client_secret) throw new Error('Missing Zoho client ID or client secret');
      ...
      console.log(JSON.stringify({ ok: true, installed: ['ZOHO_OAUTH_CLIENT_ID','ZOHO_OAUTH_CLIENT_SECRET','ZOHO_CLIENT_ID','ZOHO_CLIENT_SECRET'], env: ENV_PATH, backupCreated: fs.existsSync(backup) }));
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      res.end(html(`<p class="ok"><b>Installed.</b> Zoho OAuth credentials were written locally. You can close this tab and reply “done”.</p>`));
...
server.listen(PORT, HOST, () => {
  ...
  fs.writeFileSync('/tmp/rrd-zoho-oauth-install-url.txt', url + '\n', { mode: 0o600 });
