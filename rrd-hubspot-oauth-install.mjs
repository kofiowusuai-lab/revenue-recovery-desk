#!/usr/bin/env node
/** Local-only HubSpot OAuth credential installer.
...
const PORT = Number(process.env.RRD_HUBSPOT_OAUTH_INSTALL_PORT || 8788);
...
function html(message = '') {
  return `<!doctype html><html><head><meta charset="utf-8"><title>RRD HubSpot OAuth Installer</title>
  <style>
  ...
  </head><body><main><h1>Install HubSpot OAuth credentials</h1>${message}
  ...
  <p class="hint">Paste the values from the HubSpot developer app credentials screen for <b>Revenue Recovery Desk</b>.</p>
  <form method="post" action="/save?token=${TOKEN}">
    <label>HubSpot Client ID</label>
    <input name="client_id" autocomplete="off" spellcheck="false" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" autofocus>
    <label>HubSpot Client Secret</label>
    <input name="client_secret" type="password" autocomplete="off" spellcheck="false" placeholder="paste secret here">
    <button type="submit">Install HubSpot credentials</button>
  </form></main></body></html>`;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/save') {
      ...
      if (!client_id || !client_secret) throw new Error('Missing HubSpot client ID or client secret');
      ...
      console.log(JSON.stringify({ ok: true, installed: ['HUBSPOT_OAUTH_CLIENT_ID','HUBSPOT_OAUTH_CLIENT_SECRET','HUBSPOT_CLIENT_ID','HUBSPOT_CLIENT_SECRET'], env: ENV_PATH, backupCreated: fs.existsSync(backup) }));
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      res.end(html(`<p class="ok"><b>Installed.</b> HubSpot OAuth credentials were written locally. You can close this tab and reply “done”.</p>`));
...
server.listen(PORT, HOST, () => {
  ...
  fs.writeFileSync('/tmp/rrd-hubspot-oauth-install-url.txt', url + '\n', { mode: 0o600 });
