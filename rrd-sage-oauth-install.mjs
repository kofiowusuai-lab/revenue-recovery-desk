#!/usr/bin/env node
/** Local-only Sage OAuth credential installer.
...
const PORT = Number(process.env.RRD_SAGE_OAUTH_INSTALL_PORT || 8797);
...
function html(message = '') {
  return `<!doctype html><html><head><meta charset="utf-8"><title>RRD Sage OAuth Installer</title>
  <style>
  ...
  </head><body><main><h1>Install Sage OAuth credentials</h1>${message}
  ...
  <p class="hint">Paste the <b>Client ID</b> and <b>Client Secret</b> from your Sage Accounting developer app. The app redirect URL should include <code>https://revenue-recovery-web-ivory.vercel.app/oauth-callback</code>. If Sage allows multiple callback URLs, also add <code>https://flowaudit.co.uk/revenue-recovery/oauth-callback</code>.</p>
  <form method="post" action="/save?token=${TOKEN}">
    <label>Sage Client ID</label>
    <input name="client_id" autocomplete="off" spellcheck="false" autofocus>
    <label>Sage Client Secret</label>
    <input name="client_secret" type="password" autocomplete="off" spellcheck="false" placeholder="paste secret here">
    <button type="submit">Install Sage credentials</button>
  </form></main></body></html>`;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/save') {
      ...
      if (!client_id || !client_secret) throw new Error('Missing Sage client ID or client secret');
      const backup = upsertEnv({
        SAGE_OAUTH_CLIENT_ID: client_id,
        SAGE_OAUTH_CLIENT_SECRET: client_secret,
      });
      console.log(JSON.stringify({ ok: true, installed: ['SAGE_OAUTH_CLIENT_ID','SAGE_OAUTH_CLIENT_SECRET'], env: ENV_PATH, backupCreated: fs.existsSync(backup) }));
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      res.end(html(`<p class="ok"><b>Installed.</b> Sage OAuth credentials were written locally. You can close this tab and reply &ldquo;done&rdquo;.</p>`));
...
server.listen(PORT, HOST, () => {
  ...
  fs.writeFileSync('/tmp/rrd-sage-oauth-install-url.txt', url + '\n', { mode: 0o600 });
