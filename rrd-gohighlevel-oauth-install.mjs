#!/usr/bin/env node
/** Local-only GoHighLevel (HighLevel) OAuth credential installer.
...
const PORT = Number(process.env.RRD_GHL_OAUTH_INSTALL_PORT || 8796);
...
function html(message = '') {
  return `<!doctype html><html><head><meta charset="utf-8"><title>RRD GoHighLevel OAuth Installer</title>
  <style>
  ...
  </head><body><main><h1>Install GoHighLevel OAuth credentials</h1>${message}
  ...
  <p class="hint">Paste the <b>Client ID</b> and <b>Client Secret</b> from your GoHighLevel <b>Marketplace app</b> (Settings &rarr; Advanced Settings / OAuth) for <b>Revenue Recovery Desk</b>. Make sure the app's redirect URLs include <code>https://flowaudit.co.uk/revenue-recovery/oauth-callback</code> and <code>https://revenue-recovery-web-ivory.vercel.app/oauth-callback</code>.</p>
  <form method="post" action="/save?token=${TOKEN}">
    ...
    <button type="submit">Install GoHighLevel credentials</button>
  </form></main></body></html>`;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/save') {
      ...
      if (!client_id || !client_secret) throw new Error('Missing GoHighLevel client ID or client secret');
      const backup = upsertEnv({
        GOHIGHLEVEL_OAUTH_CLIENT_ID: client_id,
        GOHIGHLEVEL_OAUTH_CLIENT_SECRET: client_secret,
      });
      console.log(JSON.stringify({ ok: true, installed: ['GOHIGHLEVEL_OAUTH_CLIENT_ID','GOHIGHLEVEL_OAUTH_CLIENT_SECRET'], env: ENV_PATH, backupCreated: fs.existsSync(backup) }));
      res.writeHead(200, { 'content-type': 'text/html; charset=utf-8', 'cache-control': 'no-store' });
      res.end(html(`<p class="ok"><b>Installed.</b> GoHighLevel OAuth credentials were written locally. You can close this tab and reply &ldquo;done&rdquo;.</p>`));
...
server.listen(PORT, HOST, () => {
  ...
  fs.writeFileSync('/tmp/rrd-gohighlevel-oauth-install-url.txt', url + '\n', { mode: 0o600 });
